...
class TestDotenvLoading:
    ...
    def test_dotenv_loads_before_imports(self):
        ...
        init_file = Path(__file__).parent.parent.parent / "src" / "tetra_rp" / "__init__.py"
        ...
        assert dotenv_import_line < dotenv_call_line, "load_dotenv() should be called after import"
        assert dotenv_call_line < logger_import_line, "load_dotenv() should be called before logger import"
        assert dotenv_call_line < resources_import_line, "load_dotenv() should be called before resources import"
...
"""
            ...
            try:
                ...
                test_vars = [
                    "RUNPOD_API_KEY", "TETRA_GPU_IMAGE", "TETRA_CPU_IMAGE", 
                    "LOG_LEVEL", "RUNPOD_API_BASE_URL", "CUSTOM_TEST_VAR"
                ]
                original_values = {}
                ...
                assert result is True, "load_dotenv() should return True when .env file is found"
                ...
                assert os.environ.get("RUNPOD_API_BASE_URL") == "https://test-api.runpod.io"
    ...
    def test_env_vars_available_after_tetra_import(self):
        ...
        try:
            ...
            modules_to_remove = [name for name in sys.modules.keys() if name.startswith('tetra_rp')]
            ...
            modules_to_clear = [name for name in sys.modules.keys() if name.startswith('tetra_rp.core')]
            for module_name in modules_to_clear:
                del sys.modules[module_name]
            
            # Import specific modules that use environment variables
            from tetra_rp.core.api.runpod import RunpodGraphQLClient
            from tetra_rp.core.resources.live_serverless import TETRA_GPU_IMAGE, TETRA_CPU_IMAGE
    ...
    def test_missing_env_file_graceful_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert result is False, "load_dotenv() should return False when no .env file exists"
...
"""
    ...
    def test_env_vars_used_by_key_modules(self):
        ...
        try:
            ...
            modules_to_clear = [name for name in sys.modules.keys() if name.startswith('tetra_rp.core.api')]
            for module_name in modules_to_clear:
                del sys.modules[module_name]
