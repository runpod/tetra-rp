...
class FlashApp:
    ...
    @classmethod
    async def delete(cls, app_name: Optional[str] = None, app_id: Optional[str] = None) -> bool:
        if (not app_name and not app_id) or (
                app_name and app_id
                ):
    ...
    async def get_environment_by_name(self, environment_name: str):
        await self._hydrate()
        async with RunpodGraphQLClient() as client:
