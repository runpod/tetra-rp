...
2. **Environment Creation**
   - `flash deploy create <env>` calls `FlashApp.create_environment_and_app` to ensure the parent app exists and to create the environment in a single async transaction.
...
4. **Inspection & Operations**
   ...
   - `flash deploy list/get` zoom into environment state, showing associated endpoints and volumes, while `flash deploy delete` undeploys associated resources before deleting the environment (aborting on failures) with confirmation prompts.
